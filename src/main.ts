...
export interface TestHooks {
  ...
  /** Fija el estado del personaje sin transición. */
  setState(name: ClipName): void;
  /** Encuadre de cámara reproducible: 'hero' | 'profile' | 'portrait'. */
  setFraming(name: keyof typeof FRAMINGS): void;
  /** Congela el reloj para que la captura no capture un frame a medias. */
  setPaused(paused: boolean): void;
  /** Apertura de alas 0..1, saltando la rampa. */
  ...
  const hooks: TestHooks = {
    ...
    setLook(point) {
      if (point) {
        ...
        character.lookAt(lookTmp.set(point[0], point[1], point[2]));
      } else {
        director.lookEnabled = false;
